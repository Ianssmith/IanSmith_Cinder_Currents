...
class MoreParticlesApp : public App {
  public:
    ...
    const int	NUM_PARTICLES =ci::Rand::randGaussian()* 42;
    const float  RANDOM_RANGE = ci::Rand::randGaussian()*20.f;
    const float  RANDOM_FORCE_RANGE = 2.f;
    const float LIFE_SPAN = 9.f;
...
void MoreParticlesApp::mouseUp( MouseEvent event){
	misPressed=false;
}
    ...
    // init particle whenver there's particle movement
		for (int i = 0; i < NUM_PARTICLES; i++){
			mParticles.push_back(Particle::create(\
				(glm::vec2)event.getPos() + glm::vec2(ci::randFloat(-RANDOM_RANGE, RANDOM_RANGE), ci::randFloat(-RANDOM_RANGE, RANDOM_RANGE)),\
				LIFE_SPAN,\
				glm::vec2(ci::randFloat(-RANDOM_FORCE_RANGE, RANDOM_FORCE_RANGE), ci::randFloat(-RANDOM_FORCE_RANGE, RANDOM_FORCE_RANGE))\
			   ));
...
void MoreParticlesApp::draw()
{
    ...
    //ci::gl::begin(GL_LINES);
    for (auto& p: mParticles){
        ci::gl::color(ci::ColorA(p->getTimerleft()/p->getLifespan()/2, 0.5f, 2.f - p->getTimerleft()/p->getLifespan(), p->getTimerleft()/p->getLifespan()-0.1));
        // get perlin value based on position
        //vec3 norm = mPerlin.dfBm( vec3( p->getPosition().x, p->getPosition().y, mTime ) * sin(mFrequency) ) * ci::Rand::randGaussian()*20.0f;
        vec3 norm = mPerlin.dfBm( vec3( p->getPosition().x, p->getPosition().y, mTime ) * ci::Rand::randGaussian()*mFrequency ) * ci::Rand::randGaussian()*42.0f;
        vec2 flowForce = normalize( vec2( norm.x, norm.y ) ) * 0.1f;
        p->applyForce(flowForce);
        gl::vertex(p->getPosition());
        gl::vertex(p->getPosition() + glm::vec2(norm.x, norm.y) * sin(0.3f));
    }
    ci::gl::end();
