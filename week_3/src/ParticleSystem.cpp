...
////repulsionFactor = 0.05f;
	...
	for (int i = 0; i < mParticleNum; i++)
	{
		...
		Particle singleParticle( glm::vec2( ci::app::getWindowHeight()/2, 10.f ) + glm::vec2(ci::Rand::randGaussian()*range,ci::Rand::randGaussian()*range), 1.f);
		//Particle singleParticle( glm::vec2(10.f, ci::app::getWindowWidth()/2) + glm::vec2(ci::Rand::randGaussian(),ci::Rand::randGaussian()), 1.f);
		//Particle singleParticle(glm::vec2(ci::app::getWindowWidth()/2, 10.f) + glm::vec2(ci::randFloat(-range, range), ci::randFloat(-range, range)), 1.f);
